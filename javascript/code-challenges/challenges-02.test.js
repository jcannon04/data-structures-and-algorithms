...
const raisedToTheThird = (arr) => {
  let pow3Numbers = [];
  arr.forEach(number => {
    pow3Numbers.push(Math.pow(number, 3));
  });
  return pow3Numbers;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for(let number of arr) {
    twoToThes.push(Math.pow(2, number));
  }
  return twoToThes;
};
...
const evenOdd = arr => arr.map(n => Number.isInteger(n) ? ( (n % 2) === 0 ? 'even' : 'odd') : 'N/A');
...
const extractStats = (arr) => arr.map(obj => {
  return {name: obj.stat.name, total: obj.baseStat + obj.effort};
});
