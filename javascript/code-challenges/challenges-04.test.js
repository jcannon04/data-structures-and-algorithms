...
const alphabetizeBetter = (arr) => arr.sort((a, b) => a.toLowerCase() < b.toLowerCase() ? -1 : 1);
...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((a, b) => daysOfWeek[a.dayOfWeek] < daysOfWeek[b.dayOfWeek]? -1 : 1);
};
...
const sortSchedule = (arr) => {
  ...
  return arr.sort((a, b) => {
    // check day of week
    if(daysOfWeek[a.dayOfWeek] < daysOfWeek[b.dayOfWeek]) return -1;
    if(daysOfWeek[a.dayOfWeek] > daysOfWeek[b.dayOfWeek]) return 1;

    // conver start and end times to numbers for easy comparison
    let startTimes = [parseInt(a.start), parseInt(b.start)];
    let endTimes = [parseInt(a.end), parseInt(b.end)];

    // check start times
    if(startTimes[0] < startTimes[1]) return -1;
    if(startTimes[0] > startTimes[1]) return 1;

    // check end times
    if(endTimes[0] < endTimes[1]) return -1;
    if(endTimes[0] > endTimes[1]) return -1;
