...
const count = (target, input) => {
  return input.reduce((acc, innerArr) => {
    return acc + innerArr.filter(elem => elem === target).length;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((acc, elem) => {
    return acc + elem.reduce((acc, elem) => acc + elem, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(elem => (elem % 5 === 0) && (typeof elem === 'number'))
...
let findShortest = (data) => {
  ...
  return data.reduce((acc, character) => {
    if(character.height < minHeight) {
      minHeight = parseInt(character.height);
